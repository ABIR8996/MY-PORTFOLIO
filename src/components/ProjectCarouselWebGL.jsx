...
export default function ProjectCarouselWebGL() {
  ...
  const projectsList = [
    {
      ...
      tags: ['TensorFlow Lite INT8', '1D-CNN + GRU', 'Kotlin Android NNAPI', 'Sub-2ms Inference'],
      desc: 'Ultra-low latency cybernetic algorithmic trading hub & Android TFLite architecture. Full integer INT8 quantization (<150 KB) for sub-2ms local neural inference on NPU/GPU with Level 2 orderbook depth analysis.',
      image: '/assets/harsh_quant_trading_hub.jpg'
    },
    {
      ...
      image: '/assets/cybersec_osint_recon.jpg'
    },
    {
      ...
      image: '/assets/notebooklm_terminal_ai.jpg'
    },
    {
    ...
    {
      title: 'Autonomous Driving Sim & Real-Time Linux Audio DSP',
      category: 'Robotics & DSP',
      icon: <Car className="text-amber-400" size={24} />,
      tags: ['DonkeySim Linux', 'Audio DSP', 'RGB Hardware Sync'],
      desc: 'Autonomous self-driving simulation research alongside real-time Linux audio DSP scripts (sync_keyboard_bass.py) for audio-reactive hardware systems.',
      image: '/assets/robotics_audio_dsp.jpg'
    }
  ];
  ...
  return (
    <section id="projects" className="py-24 px-6 max-w-7xl mx-auto">
      <div className="text-center max-w-2xl mx-auto mb-16">
        <span className="text-xs font-mono font-bold tracking-widest text-cyan-400 uppercase">🌱 THE DEEP ROOTS OF INNOVATION</span>
        <h2 className="text-4xl font-extrabold text-white mt-2 font-mono">
          Featured <span className="bg-gradient-to-r from-cyan-400 via-purple-500 to-rose-500 text-transparent bg-clip-text">Deep Roots (Work)</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {projectsList.map((project, idx) => (
          <div
            ...
            className="group relative bg-slate-950/90 backdrop-blur-xl border border-slate-800 rounded-3xl overflow-hidden hover:border-cyan-400/60 transition-all duration-300 flex flex-col shadow-2xl hover:-translate-y-2"
          >
            ...
            <div className="p-8 flex flex-col gap-4 flex-grow">
              <div className="flex flex-wrap gap-2">
                {project.tags.map((tag, tIdx) => (
                  <span key={tIdx} className="text-[10px] font-mono px-2.5 py-1 bg-cyan-500/10 border border-cyan-400/20 text-cyan-300 rounded-lg">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              {/* Interactive Thermal Waveguide Simulator Widget */}
              {project.interactive && (
                <div className="mt-2 p-4 bg-slate-900/90 border border-slate-800 rounded-2xl">
                  <div className="flex justify-between items-center mb-2 text-xs font-mono">
                    <span className="text-cyan-400">⚡ Interactive Waveguide Phase Simulator</span>
